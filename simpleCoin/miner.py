...
def create_genesis_block():
    ...
    blk = Block(0, time.time(), {
        "proof-of-work": 9,
        "transactions": None},
        "0")
    print("Generated genesis block", blk)
...
def proof_of_work(last_proof, blockchain):
    ...
    while not (incrementer % 7919 == 0 and incrementer % last_proof == 0):
        ...
        if int((time.time()-start_time) % 60) == 0:
            ...
            # @boyl This returns true, if our chain is not longest, then skip current mining, and accept longer chain first
...
def mine(a, blockchain, node_pending_transactions):
    ...
    while True:
        ...
        # @boyl Some other node has already mined the block, proof[1] is the blockchain according to the consensus
        if not proof[0]:
            ...
            NODE_PENDING_TRANSACTIONS = requests.get(
                url=MINER_NODE_URL + '/txion', params={'update': MINER_ADDRESS}).content
            NODE_PENDING_TRANSACTIONS = json.loads(NODE_PENDING_TRANSACTIONS)
            # Then we add the mining reward

            if len(NODE_PENDING_TRANSACTIONS) == 0:
                print("No transations... skipping")
                continue
            ...
            a.send(BLOCKCHAIN)
            requests.get(url=MINER_NODE_URL + '/blocks',
...
# @boyl Returns False, if our chain is already the longest, else returns the longest chain
...
if __name__ == '__main__':
    ...
    miner_process = Process(target=mine, args=(
        pipe_output, BLOCKCHAIN, NODE_PENDING_TRANSACTIONS))
    miner_process.start()
